...
class SourceData:
    ...
    def __init__(self, file_list, non_linear=False, to_align=True, to_skip_bad=False, num_from_session=None):
    ...
    def load_headers_and_sort(self):
        ...
        # return self.timestamped_file_list

    def load_images(self, progress_bar: Optional[wx.Gauge] = None, frame: Optional[wx.Frame] = None):
        to_align = self.to_align
        non_linear = self.non_linear
        file_paths = [item[0] for item in self.timestamped_file_list]
        if progress_bar:
            progress_bar.SetRange(len(file_paths))
        ...
        if rejected_list:
            ...
            for item in rejected_list:
                self.timestamped_file_list.pop(item)
    ...
    def gen_splits(self, y_splits, x_splits, to_align=True):
        shape = self.images[0].shape
        y_shape, x_shape = shape[:2]
        y_split_size = (y_shape - 2 * self.BOARDER_OFFSET) // y_splits
        x_split_size = (x_shape - 2 * self.BOARDER_OFFSET) // x_splits
