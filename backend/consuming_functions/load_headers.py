...
def load_headers_worker(filenames: list[str], progress_queue: Optional[Queue] = None, stop_queue: Optional[Queue] = None) -> list[Header]:
    headers = []
...
def load_headers(filenames: list[str], progress_bar: Optional[AbstractProgressBar] = None,
    ...
    with Pool(available_cpu) as pool:
        ...
        results = pool.map_async(
            partial(load_headers_worker, progress_queue=progress_queue, stop_queue=stop_queue),
